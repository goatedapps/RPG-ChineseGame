...
- **Session shape:** 15–25 minutes a day, most days of the week. The game should make the child *want* to come back tomorrow, and must *not* reward playing for hours in one sitting.
...
- The **Writing** tick only counts when every character was written **from memory** (stage 3, see 3.4) without help.
...
- **Brush Fragments:** one per boss, 7 in total. Together they restore the **Spirit Brush**. Each fragment grants a map ability (this is the game's "HM" system):
...
- **Boss gate:** a set number of **Silver** spirits among the region's words (roughly 35–40% of them; Region 1: 12 of 54) **and** the region's reading key item from the Reading Hall (section 11.4). The gatekeeper shows both: "Silver spirits 9/12 · Cave Lantern: not yet".
- **Next region gate:** after the boss is beaten, the gate requires about **70% of the region's words at Gold** (the value is configurable per region). Show a progress bar on the gate sign: "Gold spirits: 21 / 38".
...
- **One passage at a time.** The Reading Hall shows one comprehension passage, picked **at random** from the level's comprehension pool (any passage, any subject; no matching to regions or difficulty). A passage is not repeated until every passage has been done once.
...
- **Export / import save:** download or upload JSON, so progress can move to another device.
...
2. Defeat the Forgetter's seven lieutenants. Each holds one **Brush Fragment**.
3. Restore the Spirit Brush and enter the Great Dictionary Tree to face the Forgetter.
...
7. **At 12 Silver and with the Cave Lantern:** the gatekeeper opens the gate to Muddle Cave.
8. **Boss: Muddle King.**
...
- The Complete Brush opens the Dictionary Tree.
...
- **Post-game:** all regions stay in review mode. Hard "Higher Chinese" questions appear as optional **Challenge Scrolls**.
...
[{ "id": "r1", "name": "Scholar Village", "lessons": [1,2,3], "tier": 1,
   ...
   "bossId": "muddle-king", "bossGate": { "silver": 12, "keyItem": "cave-lantern" }, "nextGate": { "goldPct": 0.7 },
...
- Offer export and import of the save as JSON in the parent panel. Validate `version` on import.
...
- Export and import in the parent panel use the same encoded string.
