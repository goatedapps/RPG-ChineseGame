...
export function playerAttack(battle, player, skill, { correct, random = Math.random, bonusDamage = 0, damageMultiplier = 1 } = {}) {
  ...
  const moveBonus = (skill === battle.creature.weak ? 3 : 0) + (skill === 'w' ? 2 : 0) + bonusDamage;
...
export function enemyAttack(battle, player, random = Math.random, { evasionBonus = 0, damageReduction = 0 } = {}) {
  const hero = heroStats(player.level);
  if (didEvade(hero.evasion + evasionBonus, random)) return { player, damage: 0, evaded: true };
  const damage = Math.max(0, calculateDamage({ attack: battle.creature.attack, defense: hero.defense, roll: random() * 3 }) - damageReduction);
